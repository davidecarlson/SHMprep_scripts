...
#########
# script modified by David Carlson for pRESTO analyses to compare with SHMprep using data from Stern et al. 2014
# See original authorship below
# Original script and instructions for running can be found at https://presto.readthedocs.io/en/stable/workflows/Stern2014_Workflow.html
#########
...
tail -n +2 SRA_TABLE_PE | while read line; do

START_TIME=$(date +%s)
echo $START_TIME


ID=$(echo "$line" | cut -f 11)
echo $ID
R1_FILE=$(readlink -f stern2014/${ID}_1.fastq)
echo $R1_FILE
R2_FILE=$(readlink -f stern2014/${ID}_2.fastq)
echo $R2_FILE

# Define run parameters and input files
R1_PRIMERS=$(readlink -f Primers_CRegion_Stern2014STM.fasta)
R2_PRIMERS=$(readlink -f Primers_VSegment_Stern2014STM.fasta)
OUTDIR="stern2014_out_8cpu"
OUTNAME="$ID"
NPROC=8
PIPELINE_LOG="$ID.Pipeline.log"
ZIP_FILES=false
...
echo $TOTAL_TIME > $OUTDIR/$ID.time
